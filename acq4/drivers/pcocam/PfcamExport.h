...
//           only DWORD's are returned at the moment
...
int CEXP SET_TIMEOUT_VALUES(HANDLE hdriver,DWORD *times,int len);
//Can set timout values for image,cardio,dma and head 
//in:
// hdriver = filehandle returned from INITBOARD
// dma     = timeout in ms for dma
// proc    = timeout in ms for cardio
...
int CEXP SETDRIVER_EVENT(HANDLE hdriver,int mode,HANDLE *hHeadEvent);
...
//           greater values will timeout the dma
//           or come back with error 
// adr     = pointer to external allocated image data array
// hevent  = handle of external created event
// Status  = pointer to DWORD which returns the status of the transfer

//out:
// *adr    = grabbed image data
// *Status = status of buffer

int CEXP REMOVE_BUFFER(HANDLE hdriver,void *adr);
//Clear the buffer with the given address in the buffer queue 
//If a transfer is actual in progress to this buffer
//a error is returned.
...
//Allocate a buffer for the camera in PC-Mainmemory
//or create a buffer number for a external allocated buffer
//create a event for this buffer or use a external created event
...
// size      = size of buffer in byte
// hPicEvent = handle of event
//             or NULL to create an internal event
// adr       = pointer to pointer of image data array
//             or NULL to allocate data array internal

//out:
// *bufnr     = number of buffer
// *hPicEvent = handle of event
// *adr       = pointer to image data array
...
//wait until one or more buffers are grabbed

//in:
// hdriver      = filehandle returned from INITBOARD
// nr_of_buffer = number of buffers in list
// bl           = pointer to PCC_Buflist structure(s)
//                structure(s) must be filled with valid data
// timeout      = time in ms to wait

//out:
// *bl          = version string dll

int CEXP PCC_RESETEVENT(HANDLE hdriver,int bufnr);
//Reset the Event for this buffer
